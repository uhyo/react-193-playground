...
- `FragmentInstance` exposes no "list your children" method, but `observeUsing()` accepts any `{ observe, unobserve }` object, not just real observers: react-dom calls `observe(element)` with every current first-level DOM child, again for every child mounted later, and `unobserve(element)` for every child removed; `unobserveUsing()` calls `unobserve(element)` with every current child — all during the commit phase, before paint. The "Hidden group" example leans on this to maintain the `hidden` attribute on the fragment's children with no flash of unhidden content, from a single callback ref with a cleanup: keying the callback on the `enabled` prop makes React detach the old observer and attach a fresh one whenever it flips, re-delivering every child, so no effects or child registry are needed.
